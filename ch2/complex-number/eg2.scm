...
(define (real-part z)
  (cond ((eq? (type-tag z) 'rectangular) (real-part-rectangular (contents z)))
        ((eq? (type-tag z) 'polar) (real-part-polar (contents z)))
        (else (error "Unknow type -- REAL-PART" z))))

(define (imag-part z)
  (cond ((eq? (type-tag z) 'rectangular) (imag-part-rectangular (contents z)))
        ((eq? (type-tag z) 'polar) (imag-part-polar (contents z)))
        (else (error "Unknow type -- IMAG-PART" z))))

(define (magnitude z)
  (cond ((eq? (type-tag z) 'rectangular) (magnitude-rectangular (contents z)))
        ((eq? (type-tag z) 'polar) (magnitude-polar (contents z)))
        (else (error "Unknown type -- MAGNITUDE" z))))

(define (angle z)
  (cond ((eq? (type-tag z) 'rectangular) (angle-rectangular (contents z)))
